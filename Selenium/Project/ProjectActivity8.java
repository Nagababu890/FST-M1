...
public class ProjectActivity8 {
    ...
    public void applyLeaveTest() throws Exception {
        ...
        leaveType.selectByVisibleText("DayOff"); // Choose available type
        Thread.sleep(1000);
        ...
        // Verify first row in the leave table
        WebElement leaveStatus = driver.findElement(By.xpath("//table[@id='resultTable']/tbody/tr[1]/td[6]"));
        String statusText = leaveStatus.getText();
        System.out.println("Leave status: " + statusText);

        if (statusText.contains("Pending") || statusText.contains("Scheduled")) {
            System.out.println("✅ Leave applied successfully! Status: " + statusText);
        } else {
            System.out.println("❌ Leave not applied correctly. Found status: " + statusText);
        }
    }
